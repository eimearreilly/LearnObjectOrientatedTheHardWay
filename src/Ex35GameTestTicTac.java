...
public class Ex35GameTestTicTac {
    public static void main(String [] args){
        Ex35Gameboard board = new Ex35Gameboard(3,3);
        Scanner kb = new Scanner(System.in); String p = "X";
        int r,c,turns=0;
        boolean done = false; do{
            System.out.println("\n"+board); System.out.print("'"+ p +"', your turn: ");
            r = kb.nextInt();
            c = kb.nextInt();
            if ( board.canAdd(r,c) && board.get(r,c) == null ) {
                Ex35GamePiece XO = new Ex35GamePiece(r, c, p); XO.addSelfToBoard(board);
                turns++;
                p = p.equals("X") ? "O" : "X";
        ...
        System.out.println("\n"+board);
        if ( isWinner(board, "X") ) System.out.println("X wins!");
        else if ( isWinner(board, "O") ) System.out.println("O wins!");
        else
            System.out.println("It's a tie!");
    }
    public static boolean isWinner( Ex35Gameboard board, String p ) {
    ...
    public static boolean winCheck(Ex35Gameboard board, String p, int a,int b,int c, int d, int e, int f) {
        if ( board.get(a,b) == null || board.get(c,d) == null || board.get(e,f) == null )
        return false;
