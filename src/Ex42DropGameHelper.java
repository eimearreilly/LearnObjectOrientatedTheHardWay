public class Ex42DropGameHelper
{
    private Ex41DropGamePiece[][] board;
    private int numRows, numCols;

    public Ex42DropGameHelper (int numRows, int numCols){
        board = new Ex41DropGamePiece[numRows][numCols];
    ...
    private boolean winCheck(String p, Ex41DropGamePiece piece) {
        if ( piece == null)
            return false;
        if ( ! p.equals(piece.getSymbol()) )
            return false;
        Ex41Location loc = piece.getLocation();
        if ( ! isValid(loc) )
            return false;

        Ex41Location left, right, below, above;
        int adjacentCount = 0;
        left = loc.left();
        right = loc.right();
        below = loc.below();
    ...
    public boolean isValid( Ex41Location loc ) {
    ...
    public String playerAt( Ex41Location loc ) {
    ...
    public String toString() {
        ...
        for ( int r=0; r<numRows; r++ ) {
            line = "|";
            for ( int c=0; c<numCols; c++ ) {
                Ex41DropGamePiece piece = board[r][c];
                if (piece == null)
                    line += "_|";
                else
    ...
    public void playMove( String p, int col ) {
        ...
        board[row][col] = new Ex41DropGamePiece(row, col, p);
    }
}
